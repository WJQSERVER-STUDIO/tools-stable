#!/bin/sh
# 由WJQSERVER-STUDIO的WJQSERVER自行修改, 对发行版判定进行了修改, 以适配Debian Trixie(testing)
# Modified by WJQSERVER-STUDIO, modify the script to adapt to Debian Trixie(testing)
# Script From https://get.docker.com/
# Modified by WJQSERVER-STUDIO
set -e
...
# version_gte 和 version_compare 函数保持不变
version_gte() {
	if [ -z "$VERSION" ]; then
			return 0
	fi
